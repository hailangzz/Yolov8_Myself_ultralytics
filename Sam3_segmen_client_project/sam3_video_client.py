...
class SAM3VideoClient:
    ...
    def extract_frames(self, video_path, output_dir, max_frames=None):
        ...
        while True:
            ...
            filename = os.path.join(output_dir, f"{index:06d}.jpg")
        ...
        print(f"[INFO] Extract frames: {len(frame_paths)}")
    ...
    def init_video(self, frame_paths):
        ...
        for p in tqdm(frame_paths):
            frames.append(self.image_to_base64(p))
    ...
    def add_point_prompt(self, session_id, x, y, frame_index=0, obj_id=1):

        print("[INFO] Add point prompt:", x, y)
    ...
    def propagate(self, session_id, start_frame=0, end_frame=None):

        print("[INFO] Start propagation...")

        payload = {"session_id": session_id, "model": self.model, "start_frame": start_frame, "end_frame": end_frame}
    ...
    def wait_result(self, task_id, interval=3):
    ...
    def cleanup(self, session_id):
        ...
        url = self.server + f"/v1/video/cleanup/{session_id}"

        r = requests.post(url, params={"model": self.model})
...
if __name__ == "__main__":
    ...
    client = SAM3VideoClient(SERVER)

    #
    # 1. 视频拆帧
    #

    frames = client.extract_frames("test.mp4", "./frames", max_frames=100)

    #
    # 2. 初始化
    #

    session_id = client.init_video(frames)
    ...
    client.add_point_prompt(session_id, x=300, y=200)

    #
    # 4. 开始传播
    #

    task_id = client.propagate(session_id, 0, len(frames) - 1)
