...
class SAM3VideoClient:
    ...
    def process_chunks(self, images, target, chunk_size=100, class_id=0):
        ...
        for start in range(0, total_frames, chunk_size):
            ...
            for i in range(len(chunk_images)):
                ...
                masks = prompt.get("data", {}).get("masks", [])

                if len(masks) > 0:
                    prompt_frame = i

                    print("Found object:", start + i)
            ...
            if prompt_frame < 0:
                print("No object in chunk:", start, end)
            ...
            task = self.propagate(session, len(chunk_images), start_frame=prompt_frame)
            ...
            print("Finished chunk:", start, end)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="SAM3 video server address",
    )
    ...
    images = client.load_images(args.image_dir)

    ################################
    # 执行SAM3
    ################################

    client.process_chunks(images, args.target, chunk_size=CHUNK_SIZE, class_id=CLASS_ID)
