...
def inference(req: InferenceRequest):
    ...
    results = sam3.predict(req.image_path, req.prompts)
    ...
    if result.masks is not None:
        ...
        for mask in masks:
            labels = mask_to_yolo_polygon(mask, class_id=0)
    ...
    label_path = f"/app/output/{image_id}.txt"

    save_yolo_label(label_path, yolo_labels)

    return {"status": "success", "objects": objects, "label_path": label_path}
